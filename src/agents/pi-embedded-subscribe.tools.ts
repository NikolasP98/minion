...
/**
 * Resolve the effective content array for an MCP tool result, falling back to
 ...
 *
 * When `content` is absent and `structuredContent` is present, synthesise a
 * text block from the JSON-serialised value so downstream extraction functions
 * don't silently return nothing.  When both are present, `content` already
 * carries the human-readable representation so no merge is required.
 */
