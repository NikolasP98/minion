...
const FIELD_RULES: PatternRule[] = [
  ...
  {
    pattern: /\b(token|key|secret|password|credential|api[_-]?key|auth[_-]?token|access[_-]?token|refresh[_-]?token)\b/i,
    category: "credentials",
  },
  // user_identity
  {
    pattern: /\b(email|phone|name|user[_-]?id|sender[_-]?id|author[_-]?id|account[_-]?id|username|user[_-]?name|firstname|lastname|fullname)\b/i,
    category: "user_identity",
  },
  // user_location
  {
    pattern: /\b(lat(itude)?|lng|lon(gitude)?|location|address|ip([_-]?addr(ess)?)?|geo|gps|region|city|country|zipcode|postcode)\b/i,
...
export function classifyParams(params: Record<string, unknown>): DataCategory[] {
  ...
  for (const fullKey of allKeys) {
    ...
    const leaf = fullKey.split(/[.\[\]]+/).filter(Boolean).at(-1) ?? fullKey;
